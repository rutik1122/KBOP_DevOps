try {
    ...
    function getSuccessReset(result) {
        ...
        if (flag == "Sub") {
            ...
            if (flag == "Reset") {
                ...
                //if (TypeTran == "Deposit") {
                //    if (flag == "Submit") {
                //        alert("Alert! Record posted successfully!");
                ...
                if (TypeTran == "vaultcash" || flag == "vaultcash") {
                    alert("The " + JSON.stringify(result) + " record posted successfully.");
                    getSubmit();
                }
    ...
    function getSuccessVoucher(result) {
        ...
        $.each(result, function (key, item) {
            ...
            if (result[i].instrumenttype == "S") {
                ...
                document.getElementById("beneName").value = result[i].payeename;
            }
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if (scanType == "1") {
                if (result[i].instrumenttype == "S") {
                    ...
                    var cell_1 = row.insertCell(1);
                    cell_1.innerHTML = result[i].accno;
                    row.appendChild(cell_1);
                }
    ...
    function SubmitData() {
        ...
        //alert("Alert! Data submition stpooed bt Aniket!");
        //ValidationPassed = false;

        var bene = document.getElementById("beneName").value;
        var slipname = document.getElementById("SlipFinName").innerHTML.toLowerCase();
        debugger;

        if (ValidationPassed == true && slipname != bene.toLowerCase()) {
            ValidationPassed = false;
            alert("Alert! Beneficiary Name is not matching with Account Holder name!");
        }
        ...
        if (ValidationPassed == true) {
            ...
            RefNo = "TRF" + yyyy.toString().substring(2, 4) + String(currentDate.getUTCMonth() + 1).padStart(2, '0') + String(currentDate.getDate()).padStart(2, '0') + String(currentDate.getHours()).padStart(2, '0') + String(currentDate.getMinutes()).padStart(2, '0') + String(currentDate.getSeconds()).padStart(2, '0');
            ...
            var scan_type = document.getElementById("scanning_type").value;
            ...
            var data_amt = parseFloat(chqamt);
            ...
            $.ajax({
                ...
                data: {
                    pType: ProcessType, INtellerid: gusername, response: response, transtype: 'TRF', transRefNo: RefNo, BranchSolID: BrCode, CreditAccountNo: toacc,
                    PayeeName: PName, SlipAmount: amt, ChqAmount: chqamt, recipt: recipt, DebitAccountNo: acc, chqno: chq_no_final, ChqDate: chqdate, imageUrl: ImageUrl,
                    SortCode: sort_code_final, San: san_final, transCode: trcode_final, scanType: scan_type
                },
                success: getSuccessReset,
                error: getError
            });
