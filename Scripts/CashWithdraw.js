try {
    ...
    function closeDialog() {
        ...
        $.ajax({
            ...
            data: {
                ...
                SlipAmount: amt, ChqAmount: amt, recipt: recipt, Denoms: dArray_Print, DenomCount: dCount_Print, imageUrl: ImageUrl
            },
    ...
    function SubmitData() {
        ...
        if (ch == "true") {
            //// Validation for Complete Micr Band
        ...
        if (ValidationPassed == true) {
            //debugger;
            if (document.getElementById("CheckerModule").value == "N")
                ManualInsertValidation();
            debugger;
            var checkm = document.getElementById("CheckerModule").value;
            if (ManualInsert == false && document.getElementById("CheckerModule").value == "N") {
                ...
                var sortcode = document.getElementById("sortcodeDataEntry").value;
                var san = document.getElementById("sanDataEntry").value;
                var trcode = document.getElementById("trDataEntry").value;
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    //data: {
                    //    pType: ProcessType, maip: gusername, INtellerid: gusername, response: response, transtype: transtype, transRefNo: RefNo, BranchSolID: BrCode,
                    //    imageUrl: ImageUrl, SlipAmount: amt, ChqAmount: amt, PayeeName: PName, DebitAccountNo: acc, CreditAccountNo: null, chqno: chqno, SortCode: sortcode, San: san,
                    //    transCode: trcode, ChqDate: chqDate, BankCode: bankcode
                    //},
                    data: {
                        pType: ProcessType, maip: gusername, INtellerid: gusername, response: response, transtype: transtype, transRefNo: RefNo, BranchSolID: BrCode,
                        imageUrl: ImageUrl, SlipAmount: amt, ChqAmount: amt, PayeeName: PName, DebitAccountNo: acc, CreditAccountNo: null, chqno: chqno, SortCode: sortcode, San: san,
                        transCode: trcode, ChqDate: chqDate, BankCode: bankcode, transaction_particular: trans_particular, report_code: rpt_code, remark_1: remark1, remark_2: remark2,
                        instrument_alpha: instrument_alpha
                    },
                    success: getSuccessReset,
                    error: getError
