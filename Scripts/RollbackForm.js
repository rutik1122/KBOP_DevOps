try {
    ...
    function getSuccessVoucher(result) {
        ...
        if (transtype == "CASHWDL")
            transtype = result.transactionType;
        ...
        if (transtype == "CASHDEP") {
            ...
            if (transtype == "CASHWDL01")
                document.getElementById("inst_type").value = "Cheque";
            else if (transtype == "CASHWDL02")
                document.getElementById("inst_type").value = "Withdrawal Form";
        }
    ...
    function SubmitData() {
        ...
        if (BankCode == "240") {
            ...
            if (transtype == "CASHDEP") {
                ...
                if (AccountValidation == false) {
                    document.getElementById("errorMsg").innerHTML = "Account: " + accStatus;
                    return false;
                }
            }
        }
        ...
        if (transtype == "CASHWDL" || transtype == "CASHWDL01" || transtype == "CASHWDL02") {
            ...
            if (transtype == "CASHWDL" || transtype == "CASHWDL01") {
                ...
                if (ValidateWithdrawalFormNo(document.getElementById("chqnoDataEntry").value) == false) {
                    document.getElementById("errorMsg").innerHTML = ErrorMessage;
                    return false;
                }

                var instDate = document.getElementById("ChequeDateTxt").value;
                var liveDate = currentDate();
                if (DateValidation(instDate, false) == false) {
                    //document.getElementById("ChequeDateTxt").focus();
                    document.getElementById("errorMsg").innerHTML = ErrorMessage;
                    return false;
                }
                else if (instDate != liveDate) {
        ...
        if (transtype == "CASHWDL01" && SignCheckFlag == false) {
            document.getElementById("errorMsg").innerHTML = "Signature Verification Pending!";
            return false;
        }
        ...
        if (transtype == "CASHDEP") {
            ...
            if (BankCode == "059") {
                if (outData[1] != null && outData[1] != "") {
                    if (outData[1] == "YES")
                        document.getElementById("successMsg").innerHTML = outData[0] + "; CBS Ref: " + outData[2];
                    else
                        document.getElementById("successMsg").innerHTML = outData[0];
                }
                else
                    document.getElementById("successMsg").innerHTML = outData[0];
            }
