try {
    ...
    function RecordTableRow(print) {
        ...
        if (BankCode == "765") {
            RetRow = RetRow + '<td hidden>' + print.frontgreyimagepath.replace("10.58.37.71", "ibas.aubank.in").replace("10.58.37.72", "ibas.aubank.in").replace("10.57.37.71", "ibas.aubank.in").replace("10.57.37.72", "ibas.aubank.in")
                + '</td>';
        }
        else {
            RetRow = RetRow + '<td hidden>' + print.frontgreyimagepath + '</td>';
        }

        RetRow = RetRow + '<td hidden><label id="valid_' + rowId + '">FALSE</label></td>';
    ...
    function VerifyData() {
        ...
        if (BankCode == "765") {
            ...
            if (verifyDone == true && Scanning_Type == "1") {
                if (slipCount == 0) {
            ...
            else if (verifyDone == true && (Scanning_Type == "2" || Scanning_Type == "3")) {
                if (slipCount != 0) {
    ...
    function getSuccessDownload(result) {
        debugger;
        $("#tr_cts_filedownload").show();
        // alert(result);
        //  var msg = document.getElementById("successMsg").innerHTML;
        //  document.getElementById("successMsg").innerHTML = msg +;"CTS Download Complete";

        document.getElementById("ctsMsg").innerHTML = "Successful";
    }


    function getErrorDownload(result) {
        debugger;

        $("#tr_cts_filedownload").show();
        // var msg = document.getElementById("successMsg").innerHTML;
        document.getElementById("ctsMsg").innerHTML = "Failed";
    }

    function CheckImgFlip() {
        //debugger;
        var inst_type = document.getElementById("id_inst_type").checked;
        if (inst_type == false && (Scanning_Type == "2" || Scanning_Type == "3")) {
